...
var total=0;
var Coockies=[];
function Location (name,mincustomers,maxcustomers,avgCoockies){
    this.name=name;
...
Location.prototype.randomcust= function () {
         ...
         cust.push(anything)
                      Coockies[i] = Math.floor(cust[i] * this.avgCoockies);
                 total = total + Coockies[i];
            }
        }
 ...
 pE1.appendChild(ulE1);
  ...
  for (let i = 0; i < workingHours.length; i++) {
     ...
     ulE1.appendChild(liE1); 
      liE1.textContent =`${workingHours[i]}  coockies    ${Coockies[i]}`  
  }
 liE1.textContent = `Total: ${total}`
            }
